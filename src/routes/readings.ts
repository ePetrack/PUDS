...
export default async function (app: FastifyInstance): Promise<void> {
  app.get('/readings', async (_req, reply) => {
    const rows = await query<ReadingRow>(`
      ...
      LIMIT 200`);
    const meters = await query<MeterOpt>(`
    ...
    const body = `
      ...
      <form method="post" action="/readings">
        <label>Meter <select name="meter_id" required>${meterOptions}</select></label>
  ...
  app.post<{
    ...
    const body = req.body ?? ({} as { meter_id?: string; read_at?: string; value?: string; note?: string });
    const meter_id = Number(body.meter_id);
    const value = Number(body.value);
    const read_at = (body.read_at ?? '').replace('T', ' ');
