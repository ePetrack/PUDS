...
-- Period usage between consecutive readings on the same meter.
-- The period spans (period_start, period_end]; usage is attributed to period_start
-- so a reading taken on May 1 closes out April's usage.
...
-- Monthly usage rolled up per site + utility from the reading deltas,
-- bucketed by the period_start (the month the consumption actually occurred).
